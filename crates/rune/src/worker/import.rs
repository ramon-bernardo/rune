...
use crate::grammar::{Ignore, MaybeNode, NodeAt, Remaining, Stream};
...
impl Import {
                ...
                match segment {
                    ast::ItemUseSegment::PathSegment(segment) => match segment {
                        ...
                        ast::PathSegment::Super(super_token) => {
                            ...
                            name.pop()?.ok_or_else(|| {
                                Error::new(super_token, ErrorKind::UnsupportedSuper)
                            })?;
                        }
    ...
    fn process_node(
        ...
        let (item, first, colon) = match (&self.kind, global) {
        ...
        queue.try_push_back((p.take_remaining(), item, first, colon))?;

        while let Some((p, item, first, colon)) = queue.pop_front() {
            tracing::trace!("process one");

            let result = self.handle_import(q, p, item, first, colon, add_task, &mut |p, name| {
                queue.try_push_back((p, name, false, false))
            });
    ...
    fn handle_import<'a>(
        ...
        mut colon: bool,
        add_task: &mut dyn FnMut(Task) -> Result<()>,
        enqueue: &mut dyn FnMut(Stream<'a>, ItemBuf) -> alloc::Result<()>,
    ) -> Result<()> {
        let complete = loop {
            if p.is_eof() {
                break None;
        ...
        let alias = if let Some(ast) = p.eat(K![as]).ast::<ast::Ident>()? {
            if complete.is_some() {
                return Err(Error::new(ast, ErrorKind::UseAliasNotSupported));
            }
