...
type InternalNode<'a> = syntree::Node<'a, Kind, u32, usize>;
...
impl Tree {
    ...
    #[cfg(feature = "std")]
    pub(crate) fn print_with_source(&self, source: &str) -> Result<()> {
        let o = std::io::stdout();
        let mut o = o.lock();

        syntree::print::print_with_source(&mut o, &self.inner, source).map_err(|error| {
            Error::msg(
...
impl<'a> Stream<'a> {
    ...
    pub(crate) fn take_remaining(&mut self) -> Stream<'a> {
        Stream {
            node: self.node,
...
impl<'a> Node<'a> {
    ...
    pub(crate) fn tokens<const N: usize>(&self) -> Option<FixedVec<Token, N>> {
        self.fixed_vec(|n| Token {
            span: inner_span(n.span()),
            kind: n.value(),
        })
    }
...
impl<'a> Remaining<'a> {
    /// Get the span of all remaining elements.
    pub(crate) fn span(&self) -> Option<Span> {
        self.node.is_some().then_some(self.span)
    }
...
impl<'a> MaybeNode<'a> {
    ...
    /// Extract interior children as an array if present.
    pub(crate) fn array<const N: usize>(&self) -> Option<[Node<'a>; N]> {
        match self {
            MaybeNode::Some(node) => node.nodes(),
            MaybeNode::None => None,
        }
    }
}
